...
## 最新更新
- 新增更接近 Codex 的过程区交互：`Now + Work Log`
- 工具调用、步骤推进、状态播报改为结构化事件渲染，不再只是字符串日志
- Work Log 支持折叠步骤、当前步骤高亮、长结果展开、step 级耗时显示
- `/agent` 过程展示更接近真实执行流：先播报当前状态，再展示工具动作和阶段结果
...
- 聊天消息中会显示 Codex 风格过程区：`Now` 当前状态 + `Work Log` 步骤轨迹
- Work Log 会展示结构化工具动作、结果摘要和每个步骤的处理时长
